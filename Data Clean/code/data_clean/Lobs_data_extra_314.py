...
def data_clean(start_date, end_date):
    ...
    while current_date <= end_date:
        ...
        file_path = rf"E:\mini-project\lob_cleaned\{file_name}"
        if os.path.exists(file_path):
            ...
            # 整理数据
            data_lob_final = data_lob_filtered[['Time', 'Bid1_Price', 'Bid1_Volume', 'Ask1_Price', 'Ask1_Volume',
                                                'Bid2_Price', 'Bid2_Volume','Ask2_Price', 'Ask2_Volume', 'Mid-Price',
                                                'Bid_Ask_Spread','Market_Depth_Bid', 'Market_Depth_Ask', 'Historical_Volatility',
                                                'Total_volume', 'Cumulative_Volume_Difference',
                                                'Total_Volume_Change_Corrected', 'Order_imbalance']].copy()

            # 保存文件
            output_name = f'{file_name[:-4]}.csv'
            output_csv_path = rf'E:\mini-project\lob_cleaned_extra\{output_name}'
