...
def data_clean(start_date, end_date):
    ...
    while current_date <= end_date:
        ...
        if os.path.exists(file_path):
            ...
            data['DateTime_Minute'] = data['DateTime'].dt.floor('T')

            # Group the data by the new DateTime_Minute column and aggregate various columns
            grouped = data.groupby('DateTime_Minute').agg({
                'Bid1_Price': 'mean',
                'Bid1_Volume': 'sum',
                'Ask1_Price': 'mean',
                'Ask1_Volume': 'sum',
            ...
            weighted_avg_price_mean = filtered_data.groupby('DateTime_Minute')['Weighted_Avg_Price'].mean().to_frame(
                'Trade_Price_Mean').round(2)

            # Merge the calculated mean into the aggregated data
            grouped_with_filtered_mean = pd.merge(grouped, weighted_avg_price_mean, on='DateTime_Minute', how='left')

            # Define the output file name and path
            output_name = f'{file_name[:-4]}.csv'
            output_csv_path = rf'E:\mini-project\data_combine\minute\{output_name}'
